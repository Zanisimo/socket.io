...
Manager.prototype.handleHandshake = function (data, req, res) {
  ...
  function writeErr (status, message) {
    if (data.query.jsonp && jsonpolling_re.test(data.query.jsonp)) {
      ...
      res.end(self.settings['client io object'] + '.j[' + data.query.jsonp + '](new Error("' + message + '"));');
    } else {
  ...
  this.authorize(handshakeData, function (err, authorized, newData) {
    ...
    if (authorized) {
      ...
      if (data.query.jsonp && jsonpolling_re.test(data.query.jsonp)) {
        hs = self.settings['client io object'] + '.j[' + data.query.jsonp + '](' + JSON.stringify(hs) + ');';
        res.writeHead(200, { 'Content-Type': 'application/javascript' });
      } else {
